...
import CustomizationModel from './structures/models/CustomizationModel.js'
...
export default class Vehicles extends BaseModule {
    ...
    getCustomizeDataBySession(sessionId, customize_hash) 
    {
        const session = Modules.Sessions.get(sessionId);
        if (!session) return [];

        return CustomizationModel.getAll({ user_id: session.userId, customize_hash });
    }

    getCustomizeDataByShareCode(share_code) 
    {
        return CustomizationModel.getOne({ share_code });
    }

    getSavedCustomizeData(sessionId, customize_hash) 
    {
        const session = Modules.Sessions.get(sessionId);
        if (!session) return [];
        ...
        return CustomizationModel.getAll({ _id: { $in: ids }, customize_hash });
    }

    async saveCustomizeData(sessionId, data) 
    {
        const session = Modules.Sessions.get(sessionId);
        if (!session) return undefined;
        ...
        {
            share_code = createShareCode(session);
        } while (!await CustomizationModel.ensureUnique(share_code));

        const customizationData = Object.assign({ user_id: session.userId, share_code }, data);

        return CustomizationModel.create(customizationData);
    }
    ...
    async saveProfile(sessionId, share_code) 
    {
        ...
        const { _id, user_id } = await this.getCustomizeDataByShareCode(share_code);
        if (!_id || user_id == session.userId) return false;

        const { saved_profiles } = await Modules.Users.appendCustomizeProfile({ _id: session.userId }, _id);

        return saved_profiles.includes(_id);
    }

    updateCustomizeData(sessionId, data) 
    {
        const session = Modules.Sessions.get(sessionId);
        if (!session) return undefined;

        const { share_code } = data;
        return CustomizationModel.update({ share_code, user_id: session.userId }, data);
    }
}
